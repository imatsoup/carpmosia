using System;
using System.Numerics; // Carpmosia-edit - Better map vote
    ...
    public sealed partial class VotePopup : Control
    {
        ...
        public void UpdateData()
        {
            ...
            for (var i = 0; i < _voteOptions.Length; i++) // Carpmosia-edit - Better map vote
            {
                ...
                // Carpmosia-start - Better map vote
                var opt = _voteOptions[i];
                if (entry.Icon is not null)
                {
                    opt.Button.IconL.TexturePath = entry.Icon;
                    opt.Button.IconR.TexturePath = entry.Icon;
                }
                if (entry.Preview is not null)
                {
                    opt.Preview.SetSize = new Vector2(96f, 96f);
                    opt.Preview.SetPrototype(entry.Preview.Value);
                }
                // Carpmosia-end - Better map vote
                if (_vote.DisplayVotes)
                {
                    opt.Button.IconLabel.Text = Loc.GetString("ui-vote-button", ("text", entry.Text), ("votes", entry.Votes)); // Carpmosia-edit - Better map vote
                }
                else
                {
                    opt.Button.IconLabel.Text = Loc.GetString("ui-vote-button-no-votes", ("text", entry.Text)); // Carpmosia-edit - Better map vote
                }

                if (_vote.OurVote == i)
                    opt.Button.Pressed = true; // Carpmosia-edit - Better map vote
            }
        }
