...
public abstract partial class SharedTicketPrinterSystem : EntitySystem
{
    ...
    private void OnPrint(Entity<TicketPrinterComponent> ent, ref LatheFinishPrintingEvent args)
    {
        ...
        var entProto = ProtoMan.Index(resultProto);
        if (!entProto.TryComp<TicketValueComponent>(out var ticketComp, Factory)) //does component exist from EntityProtoId of recipe result
            return;

        if (entProto.TryComp<StackComponent>(out var stackComp, Factory))//if a stack, produce tickets for each item in the stack
    ...
    private void OnReclaimed(Entity<TicketPrinterComponent> ent, ref ReclaimFinishedEvent args)
    {
        ...
        {
            if (amount <= 0 || !ProtoMan.TryIndex<MaterialPrototype>(material, out var materialProto) || materialProto.StackEntity == null) //get that material's Material Prototype
                continue;
            var entProto = ProtoMan.Index<EntityPrototype>(materialProto.StackEntity); //use that to get its entity prototype

            if (!entProto.TryComp<TicketValueComponent>(out var ticketComp, Factory) ||
                !entProto.TryComp<PhysicalCompositionComponent>(out var matphysComp, Factory)) //use that to get TicketValue and PhysicalComposition Components
